...
export const resourceTags = pgTable(
  ...
  (table) => ({
    type_name_idx: index("idx_resource_type_name").on(
      table.resource_type,
      table.resource_name
    ),
    parent_idx: index("idx_resource_parent_id").on(table.resource_parent_id),
  })
);
...
export const resourceRelationships = pgTable(
  ...
  (table) => ({
    unique_active_rel: uniqueIndex("unique_active_relationship").on(
      table.source_resource_id,
      table.target_resource_id,
      table.relationship_type
    ),
    source_idx: index("idx_source_resource").on(table.source_resource_id),
    target_idx: index("idx_target_resource").on(table.target_resource_id),
    type_idx: index("idx_relationship_type").on(table.relationship_type),
    start_idx: index("idx_start_at").on(table.start_at),
    end_idx: index("idx_end_at").on(table.end_at),
  })
);
