...
export async function extractResourceName(prompt, resources) {
    ...
    console.log('extract resource', 'ss', resources, prompt)

    const { match, confidence, reason } = await findBestMatch(resources, prompt);
    console.log(resources, 'resource', match, confidence, reason, 'ss')

    return {
        value: match,
        feedback: reason || (match
